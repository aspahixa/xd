...
exports.run = async (client, message, args) => {
  ...
  if (!allowed)
    return message.channel.send({embed:{title:` **__Greška__**`,description:`Nemaš permisiju za korištenje ove komande!`, color: 0x36393f}});

  let user = message.mentions.users.first();
  if (!user) return message.channel.send({embed:{title:` **__Greška__**`,description:`Nisi označio/la člana kom želiš skloniti warn!`, color: 0x36393f}});
  let member = message.guild.members.cache.get(user.id);

  let warns = args[1];
  if (!warns)
    return message.channel.send({embed:{title:` **__Greška__**`,description:`Nisi napisao/la broj warnova koje želiš očistiti!`, color: 0x36393f}});
  if (
    (isNaN(warns) && warns != "all") ||
    ((!isNaN(warns) && warns != "all" && warns < 1) || warns > 20)
  )
    return message.channel.send({embed:{title:` **__Greška__**`,description:`Nisi pravilno napisao/la broj warnova (1-20, all za sve)`, color: 0x36393f}});
  ...
  if (!isNaN(warns) && (warns > 0 || warns < 21)) {
    ...
    if (memberwarns === null || memberwarns == 0)
      return message.channel.send({embed:{title:` **__Greška__**`,description:`Taj član nema nijedan warn!`, color: 0x36393f}});
    if (warns > memberwarns)
      return message.channel.send({embed:{title:` **__Greška__**`,description:"Taj član nema toliko warnova (trenutno ih ima " + memberwarns + ")", color: 0x36393f}});
  ...
  if (isNaN(warns) && warns == "all") {
    let memberwarns = await db.fetch(`warns_${message.guild.id}_${user.id}`);
    if (memberwarns === null || memberwarns == 0)
      return message.channel.send({embed:{title:` **__Greška__**`,description:`Taj član nema warnove.`, color: 0x36393f}});
