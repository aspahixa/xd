...
exports.run = async (client, message, args) => {
  ...
  if (!user) return message.channel.send({embed:{title:` **__Označi člana kog želiš warnovati__**`,description:`Nisi označio/la člana kog želiš warnovati!`, color: 0x36393f}});
  ...
  if (
    ...
    return message.channel.send({embed:{title:` **__Greška__**`,description:`Ne možeš dati warn **__Administraciji__**!`, color: 0x36393f}});

  let razlog = args.slice(1).join(" ");
  if (!razlog) return message.channel.send({embed:{title:` **__Napiši razlog!__**`,description:`Nisi napisao/la razlog!`, color: 0x36393f}});

  let membername = member.nickname;
  if (membername === null) membername = user.username;

  db.add(`warns_${message.guild.id}_${user.id}`, 1);
  message.channel.send({embed:{title:` **__Warnovao/la si člana__**`,description:"Dao/la si warn članu " + membername + "!", color: 0x36393f}});
