...
exports.run = async (client, message, args) => {
  ...
  if (!member) return message.channel.send({embed:{title:` **__Greška__**`,description:`Nisi označio/la korisnika kojeg želiš odmutirati!`, color: 0x36393f}});
  ...
  if (!member.roles.cache.has(muted.id))
    return message.channel.send({embed:{title:` **__Greška__**`,description:`Taj korisnik nije mutiran!`, color: 0x36393f}});
  ...
  member.roles
    ...
    .catch(err =>
      message.channel.send({embed:{title:` **__Greška__**`,description:"Nisam mogao odmutirati tog korisnika zbog: " + err, color: 0x36393f}})
    );
};
exports.conf = {
